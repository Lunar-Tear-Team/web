...
| **Android build tools** (`apksigner`, `zipalign`) | See `android-tools-and-tailscale.md#android-build-tools` |
...
Copy or move the `.bin.e` into the release folder, but keep the original at the `cage/` root untouched.

Extract `resource_dump_android.7z` into `lunar-tear/server/assets/revisions/`.

:::tip[Tip To Save Space]
Only Revision 0 is currently necessary, so you can safely extract only folder `0` to save space.
:::
...
3. Click `Run All`.
4. Let the notebook handle everything: dependencies, APK patching, master data patching, and file downloads.
5. When the notebook finishes, it will give you a patched APK, a patched `.bin.e`, and dumped JSON in a `.zip`.
6. Place the files in their folders:
   - Put the `.bin.e` in `lunar-tear/server/assets/release/`
   - Extract the `.zip` into `lunar-tear/server/assets/master_data/`
...
This decrypts the `.bin.e` and converts it into JSON files the server can read. Column names are resolved automatically from `lunar-scripts/schemas.json`, with no external dependencies needed.
...
> This is a CMD command. If you are using PowerShell, use `$env:PYTHONUTF8 = 1;` instead of `set "PYTHONUTF8=1" &&`.
...
:::info[Why?]
This extends all content expiry dates inside the `.bin.e` so events, quests, and banners do not show as expired.

It also empties the maintenance table so the game never shows a maintenance screen.
:::
...
- For Tailscale users: your Tailscale IP
...
:::warning[Note]
You will not need `patched.apk` in the next steps.
:::

Generate a signing key one time only. Skip this if you already have `debug.keystore`.
...
Open your terminal in the `cage/` folder.
...
This generates Go code from the `.proto` files and only needs to be done once.
...
Install the Tailscale app from the Play Store, then install the patched APK.

- For the Quick Start method: transfer `patched.apk` to your phone and tap to install.
- For the manual patching method: transfer `patched_aligned.apk` to your phone and tap to install.
...
Since the emulator runs on the same PC as the server, Tailscale is not required for home use.

> If you are using the emulator on a different PC away from home, install Tailscale on that PC and it will route through automatically.
